...
@Component({
  ...
  template: `
    <ng-template #inputContainer></ng-template>
  `
})
export class DynamicFieldComponent implements OnInit, DynamicFieldModel {
  ...
  loadSelectInput = async () => {
    const {
      ...
      SelectInputModule
    } = await import("./select-input/select-input.component");
  ...
  loadTextInput = async () => {
    const {
      TextInputComponent
    } = await import("./text-input/text-input.component");
  ...
  loadDatepickerInput = async () => {
    ...
    } = await import("./datepicker-input/datepicker-input.component");
